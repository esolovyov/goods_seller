class StockTransactionsController < ApplicationController
  before_action :set_stock_transaction, only: [:show, :edit, :update, :destroy]
  ...
  # PATCH/PUT /stock_transactions/1.json
  def update
    respond_to do |format|
      if @stock_transaction.update(stock_transaction_params)
        format.html { redirect_to @stock_transaction, notice: 'Stock transaction was successfully updated.' }
  ...
  private
    ...
    def stock_transaction_params
      params.require(:stock_transaction).permit(:seller_id, :origin_warehouse_id, :destination_warehouse_id, :status, :product_id, :quantity, :invoice_uid)
    end
end
